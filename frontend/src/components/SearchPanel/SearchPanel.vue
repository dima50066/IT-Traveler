...
<template>
    <div class="absolute top-4 left-4 z-10 w-[300px]">
        <input :value="searchText" @input="$emit('update:search-text', $event.target.value)" type="text"
            placeholder="Пошук місця..." class="px-4 py-2 rounded shadow bg-white border border-gray-300 w-full" />
        <div v-if="error" class="text-red-500 mt-1">{{ error }}</div>
        <ul v-if="showDropdown && searchResults.length" class="bg-white shadow rounded mt-1 max-h-60 overflow-auto">
